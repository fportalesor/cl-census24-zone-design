...
class VoronoiProcessor(PolygonProcessor):
    ...
    def process(self, bbs_path=None, 
        ...
        if subregion is not None:
            self.data = self._assign_subregion_ids(subregion, self.subregion_id)
        ...
        if by_chunks:
            ...
            # Region chunks are created using urban areas,
            # so it is necessary to identify polygons that do not intersect those chunks
    ...
    def _prepare_input_polygons(self, buffer_filler, buffer_reduction):
        """Clean and prepare polygon geometries by smoothing edges and reducing size
